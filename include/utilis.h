...
struct Buton {
	...
	void createButton_Image(double pozx, double pozy, double size_xx, double size_yy, const string path) {
		state = ACTIVE;
		x = pozx; y = pozy;
		size_x = size_xx; size_y = size_yy;
		readimagefile(path.c_str(), x, y, x + size_x, y + size_y);
	}
};
...
struct Console {
	...
	void log(const string sir, colors col = WHITE) {
		...
		for (int i = 0; i < levelsir + 1 && i < 9; i++) {
